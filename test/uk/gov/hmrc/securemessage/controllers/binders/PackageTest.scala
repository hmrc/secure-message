...
class PackageTest extends PlaySpec {
  ...
  "languageFilterBinder" must {
    ...
    "bind with english(en) language" in {
      testBinder.bind("key", Map("language" -> Seq("en"))) must be(Some(Right(Language.English)))
    }

    "bind with welsh(cy) language" in {
      testBinder.bind("key", Map("language" -> Seq("cy"))) must be(Some(Right(Language.Welsh)))
    }

    "bind with unsupporting any other language to english(en)" in {
      testBinder.bind("key", Map("language" -> Seq("fr"))) must be(Some(Right(Language.English)))
    }
  }
}
