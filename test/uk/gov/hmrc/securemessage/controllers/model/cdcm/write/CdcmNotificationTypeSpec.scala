...
class CdcmNotificationTypeSpec extends SpecBase {

  "Json Reads" must {
    "read the json correctly" in {
      JsString("CDS-EXPORTS").as[CdcmNotificationType] mustBe CDSExports
    }

    "throw exception for invalid json" in {
      intercept[JsResultException] {
        JsString("CDS-UNKNOWN").as[CdcmNotificationType]
      }

      intercept[JsResultException] {
        JsNumber(100).as[CdcmNotificationType]
      }
    }
  }
  ...
  "Json Writes" must {
    "write the object correctly" in {
      Json.toJson(CDSExports) mustBe JsString("CDS-EXPORTS")
    }
  }
}
