...
import uk.gov.hmrc.securemessage.controllers.model.cdcm.write.CaseworkerMessage.SystemIdentifier
...
class CaseworkerMessageSpec extends AnyWordSpec with Matchers {
  "CdcmTags" must {
    "not allow empty mrn" in {
      SystemIdentifier("name", "value").asIdentifier mustBe Identifier("name", "value", None)
    }
  }
}
