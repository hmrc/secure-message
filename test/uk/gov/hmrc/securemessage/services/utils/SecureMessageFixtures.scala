...
object SecureMessageFixtures {
  ...
  def messageForSA(
    ...
    SecureMessage(
      ...
      messageType,
      validFrom = validFrom,
      List(),      alertDetails = AlertDetails(alertTemplateId, recipientName, Map()),
      alertQueue = None,
      Some(MessageDetails(formId = form, None, None, None, Some("batch-1234"), None, None)),      "testemail@email.com",
      hash = hash
    )
