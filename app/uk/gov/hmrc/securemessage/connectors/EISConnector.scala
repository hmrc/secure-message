...
import uk.gov.hmrc.securemessage.models.QueryMessageRequest
...
class EISConnector @Inject()(
  ...
  def forwardMessage(queryMessageRequest: QueryMessageRequest): Future[Either[EisForwardingError, Unit]] = {
    implicit val hc: HeaderCarrier = HeaderCarrier()
    httpClient
      .doPut[QueryMessageRequest](
        ...
        Seq(
          ...
          (CustomHeaders.CorrelationId, queryMessageRequest.requestCommon.acknowledgementReference),
      ...
      .flatMap { response =>
        response.status match {
          case NO_CONTENT =>
            val _ = auditMessageForwarded(EventTypes.Succeeded, queryMessageRequest, NO_CONTENT)
            Future.successful(Right(()))
          case code =>
            val _ = auditMessageForwarded(EventTypes.Failed, queryMessageRequest, code)
