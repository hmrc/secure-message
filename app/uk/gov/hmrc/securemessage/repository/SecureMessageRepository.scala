...
abstract class SecureMessageRepository[A: TypeTag, ID](
  ...
  private def messagesQuerySelector(identifiers: Set[Identifier], tags: Option[List[FilterTag]]) =
  ...
  protected def getMessagesCount(identifiers: Set[Identifier], tags: Option[List[FilterTag]])(
    ...
    val unreadCount = getCount(querySelector deepMerge Json.obj("readTime" -> JsNull))
  ...
  private def getCount(selectorObj: JsObject)(implicit ec: ExecutionContext) =
    collection.count(
