...
class AddMessageToConversationISpec extends ISpec {
  ...
  "A POST request to /secure-messaging/conversation/{client}/{conversationId}/customer-message" must {
    ...
    "return NOT FOUND when the conversation ID is not recognised" in {
      ...
      response.body mustBe "\"Error on conversation with client: Some(CDCM), message id: D-80542-20201120, error message: Conversation not found for identifier: Set(Identifier(EORINumber,GB1234567890,Some(HMRC-CUS-ORG)))\""
    }

    "return NOT_FOUND when the customer is not a participant" in new CustomerTestCase("GB1234567891") {
      response.status mustBe NOT_FOUND
      response.body mustBe "\"Error on conversation with client: Some(CDCM), message id: D-80542-20201120, error message: Conversation not found for identifier: Set(Identifier(EORINumber,GB1234567891,Some(HMRC-CUS-ORG)))\""
    }
  }
  ...
  "A POST request to /secure-messaging/conversation/{client}/{conversationId}/caseworker-message" must {
    ...
    "return NOT FOUND when the conversation ID is not recognised" in {
      ...
      val response =
        wsClient
          ...
          .futureValue
      response.status mustBe NOT_FOUND
      response.body mustBe "\"Error on conversation with client: Some(CDCM), message id: D-80542-20201120, error message: Conversation not found for identifier: Set(Identifier(CDCM,D-80542-20201120,None))\""
    }
    "return BAD_REQUEST when invalid message content is supplied" in new CaseworkerTestCase(
      ...
      response.body mustBe "\"Error on conversation with client: Some(CDCM), message id: D-80542-20201120, error message: Html contains disallowed tags, attributes or protocols within the tags: matt. For allowed elements see class org.jsoup.safety.Whitelist.relaxed()\""
    }
  }
