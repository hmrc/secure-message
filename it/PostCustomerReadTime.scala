...
class PostCustomerReadTime extends PlaySpec with ServiceSpec with BeforeAndAfterEach {
  ...
  "A POST request to /secure-messaging/conversation/{client}/{conversationId}/read-time" should {
    ...
    "return CREATED when read time added to DB" in {
      ...
      val response =
        wsClient
          ...
          .withHttpHeaders(AuthUtil.buildEoriToken, (HeaderNames.CONTENT_TYPE, ContentTypes.JSON))
    ...
    "return BAD REQUEST when read time not added to DB" in {
      ...
      val response =
        wsClient
          .url(resource("/secure-messaging/conversation/cdcm/D-80542-20201120/read-time"))
          .withHttpHeaders(AuthUtil.buildNonEoriToken, (HeaderNames.CONTENT_TYPE, ContentTypes.JSON))
          .post(new File("./it/resources/read-time.json"))
          .futureValue
